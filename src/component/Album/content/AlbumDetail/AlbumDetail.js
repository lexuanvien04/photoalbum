...
const AlbumDetail = () => {
    ...
    React.useEffect(()=>{
        let id = params.id;
        axios.get('https://photo.azurecloud.vn/api/v1/albums/detail/'+id+'?&sortBy=DESC&per_page=12') 
        .then(json=>{
            console.log(json.data.data.album)
            let medias = json.data.data.album;
            setDetails(medias);

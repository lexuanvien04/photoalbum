...
const AlbumItem = (props) => {
    ...
    useEffect(()=>{
        axios.get('https://photo.azurecloud.vn/api/v1/albums/list-albums-web?per_page=12&time=DESC')
        .then(json =>{
            console.log(json.data.data.albums.data)
          let t = json.data.data.albums.data;
          setItems(t);
                            ...
                            <MDBRipple rippleColor='light' rippleTag='div' className='bg-image hover-overlay'>
                                <Link to={"/album-detail/"+item.id}>
                                    <div className="img-item">
                                        <MDBCardImage src={item.image} fluid alt='...'  key={item.id}/>
                                    </div>
